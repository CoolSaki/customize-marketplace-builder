...
import { LoadingState, loadingReducer } from 'unicial-dapps/dist/modules/loading/reducer'
          ...
          if (item.collectionId === collection.id && item.urn) {
            ...
            if (collectionURN.type === URNType.COLLECTIONS_THIRDPARTY) {
              ...
              if (itemURN.type !== URNType.COLLECTIONS_V2) {
                throw new Error(`The item ${item.id} is not part of a unicial collection but it should be`)
              }
